...
<?php 
	...
	*/
	class product
	{
		...
		public function insert_product($date,$files){
			...
			if($product_code ='' || $productName == "" || $productQuantity == "" || $category == "" || $brand == "" || $product_desc == "" || $price == "" || $type == "" || $file_name == ""){
				...
				if($result){
					...
					$alert = "<span class='error'>Chèn sản phẩm thất bại</span>";
					return $alert;
				}
			}
		}
		...
		{
			$tukhoa = $this->fm->validation($tukhoa); //gọi ham validation từ file Format để ktra
